...
const createRecurringExpenseFormSchema = (
...
export function RecurringExpenseForm({
  ...
  const formSchema = createRecurringExpenseFormSchema(t);
  ...
  const {
    ...
    defaultValues: {
      ...
      currency: recurringExpense?.currency || accountCurrency,
      summary: recurringExpense?.summary || "",
      description: recurringExpense?.description || "",
      category_id: recurringExpense?.category_id || undefined,
      frequency: recurringExpense?.frequency || "monthly",
      custom_interval: recurringExpense?.custom_interval || 1,
      custom_unit: recurringExpense?.custom_unit || "days",
      day_of_week_mask: recurringExpense?.day_of_week_mask || 0,
      day_of_month: recurringExpense?.day_of_month || null,
      start_date:
        recurringExpense?.start_date || format(new Date(), "yyyy-MM-dd"),
  ...
  // Calculate past occurrences count (only for new recurring, not edits)
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  const pastOccurrencesCount =
    !recurringExpense && previewDates.length > 0
  ...
  const onSubmit = async (data: RecurringExpenseFormData) => {
    ...
    try {
      const url = recurringExpense
        ? `/api/recurring-expenses/${recurringExpense.id}`
        : "/api/recurring-expenses";
      const method = recurringExpense ? "PUT" : "POST";
    ...
    >
      ...
      <div className="scrollbar-thin min-h-0 flex-1 space-y-4 overflow-y-auto pr-2">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="space-y-2">
            ...
            <Select
              defaultValue={recurringExpense?.currency || accountCurrency}
          ...
          <div className="space-y-2">
            ...
            <Select
              defaultValue={recurringExpense?.category_id || "none"}
              onValueChange={(value) =>
      ...
      <div className="flex shrink-0 justify-end gap-2 border-t bg-background pt-4">
        ...
        <Button type="submit" disabled={isSubmitting}>
          ...
          {recurringExpense ? t("updateRecurring") : t("addRecurring")}
        </Button>
      </div>
    </form>
  );

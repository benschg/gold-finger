...
import { createRecurringIncomeSchema } from "@/lib/validations/schemas";
...
import { catchUpRecurringIncome } from "@/lib/recurring-generator";
...
export async function GET(request: Request) {
  ...
  if (error) {
    return NextResponse.json(
      { error: sanitizeDbError(error, "GET /api/recurring-incomes") },
      { status: 500 },
    );
  }
...
export async function POST(request: Request) {
  ...
  const validation = validateRequest(createRecurringIncomeSchema, json);
  ...
  if (error) {
    return NextResponse.json(
      { error: sanitizeDbError(error, "POST /api/recurring-incomes") },
      { status: 500 },
    );
  }
  ...
  if (firstOccurrenceDate <= today) {
    ...
    try {
      await catchUpRecurringIncome(supabase, recurringIncome.id);

      // Fetch the updated recurring income to return current state
      ...
      return NextResponse.json(updatedRecurring || recurringIncome, {
        status: 201,
      });
    } catch (catchUpError) {
      console.error("Error during catch-up generation:", catchUpError);
      // Still return the created recurring income even if catch-up failed
    }
  }
